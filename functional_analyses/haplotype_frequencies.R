...
#calculate functionally significant haplotype frequencies
...
#input for each variable comes from saved files in top_go.R
#lfmm64_input.Rdata is the genotype matrix returned by get_genotype_matrix.R
...
rm(list=ls())
load("lfmm64_input.Rdata")
load("winter_sig.gos.cats.Rdata")
load("GO.DF.Winter.Rdata")

#winter_sig.gos=winter_sig.gos[which(winter_sig.gos$elimKS <= 0.01),]
winter_sig.gos=winter_sig.gos[winter_sig.gos$elimKS %in% head(sort(winter_sig.gos$elimKS), 3),]

goterm = winter_sig.gos$GO.ID
maketab = GO.DF.SAMPLES[which(GO.DF.SAMPLES$go_acc %in% goterm),]
write.table(maketab,file="sst.wint_GO2prot_supplemental.tsv",col.names = T,sep="\t")
protloc = GO.DF.SAMPLES[which(GO.DF.SAMPLES$go_acc %in% goterm),]$proteinId
snpPosFil<-hit.DF.SAMPLES[ hit.DF.SAMPLES$proteinId%in% protloc, ] #get locations of protein IDs matched with GO terms
...
#get list of columns to apply haplotype paste
table(substr(colnames(genotypesterm),1,12))

repd = "SCAF_15.1073"
hap15 = data.frame("hap15" = apply( genotypesterm[ , substr(colnames(genotypesterm),1,12) %in% repd ] , 1 , paste , collapse = "" ))

repd = "SCAF_10.6"
hap10 = data.frame("hap10" = apply( genotypesterm[ , substr(colnames(genotypesterm),1,9) %in% repd ] , 1 , paste , collapse = "" ))

gen = genotypesterm[,c(1,2,7)]
colnames(gen) = c("SCAF_5a","SCAF_5b","SCAF_15.10702287") 

genotypesterm = cbind(gen,hap10,hap15)
...
for (i in seq_along(snpbypop[[4]])) {
  snpbypop[[4]][[i]]$site = sub("\\..*", "", names(snpbypop[[4]][i]))
  snpbypop[[4]][[i]]$geno = paste0(sub("\\..*", "", names(snpbypop)[4]),".",snpbypop[[4]][[i]]$x)
  snpbypop[[4]][[i]]$position = sub("\\..*", "", names(snpbypop)[4])
}

for (i in seq_along(snpbypop[[5]])) {
  snpbypop[[5]][[i]]$site = sub("\\..*", "", names(snpbypop[[5]][i]))
  snpbypop[[5]][[i]]$geno = paste0(sub("\\..*", "", names(snpbypop)[5]),".",snpbypop[[5]][[i]]$x)
  snpbypop[[5]][[i]]$position = sub("\\..*", "", names(snpbypop)[5])
}
...
b=bind_rows(snpbypop[[2]])
c=bind_rows(snpbypop[[3]])
d=bind_rows(snpbypop[[4]])
e=bind_rows(snpbypop[[5]])

pie.df=rbind(a,c,d,e)
pie.df$sps = ifelse(pie.df$site == "SPS" & pie.df$Freq > 0.5, "sps.max",
                    ifelse(pie.df$site == "SPS" & pie.df$Freq < 0.5 & pie.df$Freq > 0, "sps.min",NA))

sps.max=pie.df[which(pie.df$sps == "sps.max"),]$geno
pie.df$sps[pie.df$geno %in% sps.max] <- "sps.max"
sps.min=pie.df[which(pie.df$sps == "sps.min"),]$geno
pie.df$sps[pie.df$geno %in% sps.min] <- "sps.min"
pie.df$sps[is.na(pie.df$sps)] <- "a"

b$sps = ifelse(b$geno == "SCAF_5b.b","sps.max","sps.min")
pie.df = rbind(pie.df,b)

pie.df$site = factor(pie.df$site,levels=c("SPS","WB","SJF"))
pie.df$sps = factor(pie.df$sps,levels=c("sps.max","sps.min","a"))

sst.pie.df = pie.df
#save(sst.pie.df,file="sst.pie.df.Rdata")

ggplot(sst.pie.df,aes(x="",y=Freq,group=site,fill=sps))+
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y")+
  theme_classic()+
  scale_fill_manual(values=c("#FF6E6F","#00C2C3","#8F6FFD"))+
  theme(axis.text = element_blank(),
        axis.ticks = element_blank(),
        panel.grid  = element_blank(),
        panel.border=element_blank(),
        ...
        plot.background = element_rect(fill='transparent', color=NA),
        legend.position = "none")+
  theme(panel.spacing.x=unit(-1.2, "lines"),panel.spacing.y=unit(-.4, "lines"))+
  facet_wrap(~ position + site,ncol=3)

sstw.maketab = maketab
sstw.snpPosFil = snpPosFil
save(sstw.maketab,sstw.snpPosFil,file="sstwinter_tabs4sup.Rdata")
...
rm(list=ls())
load("lfmm64_input.Rdata")
load("kd490winter_sig.gos.cats.Rdata")
load("GO.DF.kd490winter.Rdata")

kd490winter_sig.gos=kd490winter_sig.gos[which(kd490winter_sig.gos$elimKS <= 0.01),]

goterm = kd490winter_sig.gos$GO.ID
maketab = GO.DF.SAMPLES[which(GO.DF.SAMPLES$go_acc %in% goterm),]
write.table(maketab,file="kd490wint_GO2prot_supplemental.tsv",col.names = T,sep="\t")
protloc = GO.DF.SAMPLES[which(GO.DF.SAMPLES$go_acc %in% goterm),]$proteinId
snpPosFil<-hit.DF.SAMPLES[ hit.DF.SAMPLES$proteinId%in% protloc, ] #get locations of protein IDs matched with GO terms
...
#get list of columns to apply haplotype paste
table(substr(colnames(genotypesterm),1,9))

repd = "SCAF_18.7"
hap18 = data.frame("hap18" = apply( genotypesterm[ , substr(colnames(genotypesterm),1,9) %in% repd ] , 1 , paste , collapse = "" ))
gen = genotypesterm[,-c(5,6)]
colnames(gen) = c("SCAF_2.15079216" ,"SCAF_7.5260199" , "SCAF_106", "SCAF_108")

genotypesterm = cbind(gen,hap18)
snpbypop=apply(genotypesterm,2,function(y){ tapply(y,GenGroupK3,function(x){data.frame(table(x)/length(x))})}) #freq of each SNP by cluster
...
pie.df=rbind(a,b,c,d,e)

#add column to set color scheme later
pie.df.nohap = pie.df[pie.df$position != "hap18",]
pie.df.nohap$sps = ifelse(pie.df.nohap$site == "SPS" & pie.df.nohap$Freq > 0.5, "sps.max",
                          ifelse(pie.df.nohap$site == "SPS" & pie.df.nohap$Freq < 0.5 & pie.df.nohap$Freq > 0, "sps.min",NA))

sps.max=pie.df.nohap[which(pie.df.nohap$sps == "sps.max"),]$geno
pie.df.nohap$sps[pie.df.nohap$geno %in% sps.max] <- "sps.max"
sps.min=pie.df.nohap[which(pie.df.nohap$sps == "sps.min"),]$geno
pie.df.nohap$sps[pie.df.nohap$geno %in% sps.min] <- "sps.min"

pie.df18 = pie.df[pie.df$position == "hap18",]
pie.df18$sps = ifelse(pie.df18$geno == "hap18.bb","sps.max",
                      ifelse(pie.df18$geno == "hap18.aa","sps.min","a"))


pie.df = rbind(pie.df.nohap,pie.df18)

pie.df$site = factor(pie.df$site,levels=c("SPS","WB","SJF"))
pie.df$sps = factor(pie.df$sps,levels=c("sps.max","sps.min","a"))

kd490w.pie.df = pie.df
#save(kd490w.pie.df,file="kd490w.pie.df.Rdata")

ggplot(pie.df,aes(x="",y=Freq,group=site,fill=sps))+
  geom_bar(width = 1, stat = "identity") +
  coord_polar("y")+
  theme_classic()+
  scale_fill_manual(values=c("#FF6E6F","#00C2C3","#8F6FFD"))+
...
# kdw.maketab = maketab
# kdw.snpPosFil = snpPosFil
# save(kdw.maketab,kdw.snpPosFil,file="kd490winter_tabs4sup.Rdata")
